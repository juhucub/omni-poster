...
| Regression tests for critical rendering and TTS behavior | Partial | `python3 -m pytest backend/app/tests/test_vertical_slice.py` passed with `93 passed` on 2026-05-11 | Coverage includes voice manifest propagation, provider fail-closed behavior, persisted segment WAV assembly, render layout snapshots, preview settings persistence, portrait/background storage separation, background preset discovery, XTTS runtime/cache metadata, OpenVoice render-profile stages, and staged Voice Lab upload memory regression protection. |
