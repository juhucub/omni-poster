...
| Regression tests for critical rendering and TTS behavior | Partial | `python3 -m pytest backend/app/tests/test_vertical_slice.py` passed on 2026-05-05 | Added coverage for voice manifest propagation, reference audio validation, processed reference artifacts, calibration matrix preview metadata, saved calibration recipe render snapshots, OpenVoice fail-closed behavior, render TTS error handling, TTS metadata exposure, persisted segment WAV artifact URLs, dialogue composite WAVs, final extracted audio metadata, and final MP4 audio assembly from segment WAVs. |
...
| Voice Lab | Partial | `backend/app/tests/test_vertical_slice.py::test_reference_audio_upload_normalizes_audio_and_invalidates_embedding`; `test_reference_audio_upload_accepts_soft_warning_metadata`; `test_openvoice_prepare_voice_profile_prefers_processed_reference_path`; `test_voice_lab_calibration_matrix_queues_recipe_previews`; `test_saved_calibration_recipe_is_snapshotted_for_video_render`; `npm run build` passed on 2026-05-05 | Voice Lab persists original/processed reference artifacts, validation metadata, fixed test phrase controls, preview WAV links, calibration matrix batches, unsupported-control reporting, and selected recipe save-back; full manual runtime UX verification still needed. |
