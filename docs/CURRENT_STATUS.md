...
- Format-aware script generation now returns structured `GeneratedScript` objects through `POST /script-generation/generate`, supports local Ollama via configurable `OLLAMA_*` settings, and falls back to deterministic hardcoded templates for reddit stories, character dialogue, podcast clips, debate, meme news reaction, educational shorts, and multi-speaker skits. Generated scripts persist on script revisions while derived speaker lines continue to feed TTS/render jobs.
...
- Added local-first Ollama script generation with deterministic fallback, hardcoded format templates, platform pacing rules, script normalization/validation, caption block generation, generated-script persistence, Project Editor controls, optional Docker Compose Ollama profile, and regression coverage. `python3 -m pytest backend/app/tests/test_script_generation.py` passed with `7 passed`, `python3 -m pytest backend/app/tests/test_vertical_slice.py` passed with `96 passed`, `python3 -m compileall backend/app` passed, and `npm run build` passed on 2026-05-13.
