...
def split_audio_whisper(audio_path, audio_name, target_dir='processed', device='cuda'):
    ...
    runtime_device, compute_type = _whisper_runtime(device)
    runtime = (runtime_device, compute_type)
    if model is None or model_runtime != runtime:
        model = WhisperModel(model_size, device=runtime_device, compute_type=compute_type)
